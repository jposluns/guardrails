...
def _render_toml(rows, prov):
    """Render the cwe.toml text from the CROSS-MAPPED SUBSET rows plus the provenance dict. `rows` is the
    filtered set actually vendored (only the weaknesses the pack's rules cite); the provenance counts
    (total elements, live count, published figure, source shas) describe the FULL pinned source and are
    preserved verbatim, so trimming the shipped id set never weakens the source provenance. LF-only."""
    ...
    header = [
        ...
        "# kind=weakness, status=snapshot: pinned to a content edition. catalogue=subset: only the",
        "# {} CWE weaknesses the pack's rules cross-map, drawn from the live (non-deprecated,".format(
            vendored),
        "# non-obsolete) CWE {} Weakness set. See README.md.".format(prov["version"]),
        ...
        "{} vendored (cross-mapped subset).".format(
            prov["total_weakness_elements"], breakdown, prov["active_after_filter"], vendored),
...
def mapped_cwe_ids(root):
    """The set of CWE ids the pack's rules actually cross-map, read from the rule-corpus frontmatter
    (`map-cwe-tight` / `map-cwe-broad`) via the repo's own corpus loader. This is the authoritative
    'what we map' set the vendored subset is filtered to, so the shipped manifest carries exactly the
    referenced weaknesses and no complete non-commercial catalogue. Offline, stdlib + repo only."""
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    import gen_rules  # noqa: E402  (the one frontmatter parser + full corpus validation)
    ids = set()
    for _src, fm, _rel in gen_rules.load_corpus(Path(root) / ".aiqt" / "core" / "rules"):
        for key in ("map-cwe-tight", "map-cwe-broad"):
            for cid in fm.get(key, []):
                ids.add(cid)
    return ids
...
def render(staging_dir, output, *, mapped_ids=None, expected_member=EXPECTED_MEMBER,
    ...
    # Filter the full live set down to the weaknesses the pack's rules cross-map, so the shipped manifest
    # is a curated subset (catalogue=subset), never a complete non-commercial catalogue. The source
    # provenance above (shas, full/live/published counts, reconcile) is validated against the WHOLE source
    # and preserved unchanged in the header; only the emitted [[id]] set is trimmed. A mapped id that is
    # absent from the parsed catalogue is a hard fail-closed abort (a stale mapping must surface, never
    # silently drop). mapped_ids defaults to the live rule corpus at the repo root.
    if mapped_ids is None:
        mapped_ids = mapped_cwe_ids(Path(__file__).resolve().parent.parent)
    subset = [row for row in rows if row[1] in mapped_ids]
    missing = set(mapped_ids) - {row[1] for row in subset}
    if missing:
        raise ImportError_("mapped CWE id(s) absent from the parsed 4.20 catalogue: {}; STOP"
                           .format(sorted(missing)))
    vendored = len(subset)
    if vendored < 1:
        raise ImportError_("no cross-mapped CWE ids resolved; refusing an empty manifest")
    ...
    text = _render_toml(subset, trusted)
    # Round-trip: parse the rendered TOML, then a real load_manifests over a temp dir, BEFORE writing.
    parsed = tomllib.loads(text)
    if len(parsed.get("id", [])) != vendored:
        raise ImportError_("rendered [[id]] count {} != vendored subset rows {}".format(
            len(parsed.get("id", [])), vendored))
    out = Path(output)
    ...
    try:
        ...
        raise
    print("[render] wrote {} ({} vendored cross-mapped weakness rows of {} live, edition {})".format(
        out, vendored, active, version))
...
def _verify_with_loader(toml_path):
    ...
    try:
        ...
        if man is None or man.catalogue != "subset" or man.kind != "weakness":
            raise ImportError_("loaded manifest is not the expected map-cwe/subset/weakness")
    finally:
...
def self_test():
    ...
    try:
        ...
        # The subset filter is exercised with an explicit mapped set (offline, no rule corpus needed):
        # keep all three fixture weaknesses so the happy path renders 3 rows.
        _mapped_all = {"CWE-1", "CWE-59", "CWE-363"}
        ...
        _expect_fail("render fails closed on a mapped id absent from the catalogue",
                     lambda: render(_stage, _stage / "cwe-missing.toml", mapped_ids={"CWE-59", "CWE-99999"},
                                    expected_xml_sha256=_sha256(_xml), expected_total=5, expected_active=3,
                                    expected_published=3, expected_retrieved="2026-04-30", min_elements=1))
        print("  ok (subset filter): narrower mapped set vendors only cited rows; absent mapped id fails closed")
        ...
        try:
            _expect_fail("render fails closed when the loader rejects the candidate",
                         lambda: render(_stage, _dest2, mapped_ids=_mapped_all,
