...
def _no_ctrl(value, where):
    """Reject an en dash, em dash, control character, or an embedded newline in an emitted display string,
    fail-closed (ValueError -> exit 2), so a bad source surfaces here rather than as a red site or dash
    gate on the generated output. A tab or newline in a Markdown table cell would also break the row."""
...
def _fence_for(residue):
    ...
    residue token can close the fence early. A residue that cannot be fenced (a control character the
    _no_ctrl guard already rejects) never reaches here."""
...
# must survive to the block's visible text, not swallow it). The hook residue carries a THREE-backtick run,
# so the Markdown fence must lengthen to four backticks to wrap it without breaking out.
...
def self_test_main():
    ...
    try:
        ...
        # (r) An en dash in an emitted display string (a ledger residue) fails closed via _no_ctrl.
    ...
    print("SELF-TEST PASS: a conformant tree generates and regenerates drift-clean with enforced, none, "
          ...
          "mechanism's ledger residual rendered VERBATIM (a tag-shaped token survives as visible text and "
          "a three-backtick run is wrapped in a lengthened fence); the display-fidelity assertion fails "
          "closed on a truncated residual block or a residue missing from the Markdown; a drifted Markdown "
          "target AND a hand edit to the generated page chrome each fail --check (exit 1); and an unknown "
          "roadmap top-level or row key, a missing or extra roadmap row, an enforced/none/mechanism "
          "mismatch against the ledger linkage, a none row with a description, a pending row missing its "
          "description or carrying a mechanism or carrying only whitespace, an unsupported lint: "
          "reference, a stale committed ledger, an emitted en dash in a residue, a boolean roadmap "
          "version, a site shell missing its content token, and an invalid-UTF-8 generated Markdown or "
          "HTML target all fail closed (exit 2)")
    return 0
