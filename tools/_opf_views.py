...
def _toml_header(source_blobs):
    ...
    newline; every value passes through _html_comment_safe (reused here as a newline/control scrubber), which
    collapses newlines/CR and drops other C0/DEL controls, so no value can forge a header field line."""
    safe = _html_comment_safe
    sources = ", ".join(safe(p) for p in sorted(source_blobs)) if source_blobs else "(none)"
    return (
        "# GENERATED by {cmd} (tools/_opf_views.py). DO NOT EDIT; edit the store and regenerate.\n"
