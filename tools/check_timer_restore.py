...
def _walk_no_nested(nodes):
    ...
    nested def/lambda/class is yielded but never entered, so a binding inside it is not attributed to the
    ...
    lambda body and a class body are SKIPPED and not separately analysed."""
    stack = list(nodes)
    while stack:
        n = stack.pop()
        yield n
...
def _analyze_scope(scope, resolver, bound_names):
    ...
    def visit_expr(node, armed):
        ...
        which is not guaranteed to match source order, for monotonic/arm/restore calls and walrus saves;
        returns the updated path armed state. Never descends into a nested function, lambda, or class body."""
    ...
    def visit_stmt(node, armed):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.Lambda, ast.ClassDef)):
            return armed                          # a nested scope, never entered here (a def body is its own
                                                  ...
                                                  # are SKIPPED, not analysed, so their statements are not
                                                  # folded into this enclosing scope's flow)
...
# class-body save is never recorded; this mixed shape is what exercises the exclusion.)
