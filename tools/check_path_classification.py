...
Advisory scan for an unsafe classification of control and state paths. Code that decides what to do about a
control or state path (a lock, marker, journal, or state directory) by a FOLLOWING existence or type check
on the string name, then proceeds as if a negative or wrong-type result meant nothing to do, both follows a
symbolic link an attacker can swap in and collapses a wrong-type or unreadable entry into benign absence.
The corpus rule secspr (security-seci-symlink-resolution, its control-path-classification clause) requires
the classification to be a no-follow inspection bound to a trusted directory descriptor, with a three-way
result: genuine absence, presence of exactly the expected type confirmed on the opened object, and
everything else refused as an error rather than collapsed into absence; and its sibling fail-closed and
no-silent-empty rules (secfcl, chkfcl) require a wrong-type or unreadable entry to surface rather than read
as clean.

This gate is a WARN-only v1 best-effort heuristic, not a decision procedure and not a specification of its
own behaviour. It scans a recognizable subset of the repo's own Python (AST plus a local, per-scope
...
flagging them for human review against secspr and chkfcl/secfcl. A recognized scope is reported at exit 0.
Recognition is syntactic and per-scope, and it applies internal suppression and downgrade heuristics: for
example it can recognize an explicit raise on a negative branch, and no-follow evidence, which can suppress
or downgrade a finding. It does not promise a WARN for each risky case or silence for a particular shape: a
shape it does not model may emit or may go unflagged, so it can both miss a genuine collapse and, on
dataflow it does not follow (for example an `import ... as <name>` rebind, an unproven receiver provenance,
a classify that is only part of a larger BoolOp, a cross-function or cross-module flow, dynamic dispatch, or
getattr indirection), emit a false-positive advisory; each is tolerable under WARN-only. The detector
...
cannot-evaluate rather than followed. Both directories are REQUIRED: an unreadable or absent one is a
cannot-evaluate.

The classifier table is Python-first; further languages are additive rows plus a parser, a named follow-on,
outside this scope. The secspr and chkfcl/secfcl rules carry the full obligation.
...
def _interpreter_isolated(flags):
    """True when a sys.flags-like object reports isolated mode: -I, or the full -P -E -s equivalent."""
...
NOFOLLOW_METHODS = frozenset({"lstat", "is_symlink"})          # not a following-classify
...
class _Resolver:
    ...
    def scoped(self, shadowed):
        ...
        constructor names that a local binding SHADOWS removed, so `def make(os): os.makedirs(...)`
...
def _is_path_ctor_call(call, resolver):
    """True when `call` resolves, against the resolver it is given, to a pathlib constructor: a bare
...
def _is_proven_path(node, path_names, resolver):
    """True when `node` is recognized as a filesystem path object: a pathlib constructor call, a
...
def _proven_path_names(func, resolver):
    ...
    # This provenance shrink is best-effort, not a tier promise: a rebind this scan does not model (an
...
def _classify_is_certain_fs(call, resolver, path_names):
    """True when a following-classify `call` is a CERTAIN filesystem operation: an os.path.<exists/isdir/
...
def _walk_no_nested(nodes):
    ...
    def/lambda is yielded (visible as a statement) but not entered, so a statement inside a nested
...
def _state_change_targets(nodes, resolver, path_names):
    ...
    filesystem change. Recognition here is a best-effort heuristic, not a promise (the builtin `open`
...
def scan(files):
    """Scan captured (relpath, raw-bytes) inputs. Returns (errors, deny, warn) sorted. A decode or parse
    failure on a captured input is a cannot-evaluate (fail-closed)."""
    errors, deny, warn = [], [], []
    for si in files:
        rel = si.relpath
        raw = si.raw
...
def run(root):
    ...
    the code and the --self-test. WARN-only v1: the certain collapse shapes and the resist-static shapes are
    both emitted as WARN advisories for human review. A scanned input yields exit 0 (ran, with or without
    advisories) or exit 2 (cannot-evaluate on an unreadable, unparseable, or over-capacity input); a scanned
    input does not drive exit 1. The per-file pipeline fails closed to a located cannot-evaluate (see
    scan()) and advisory rendering is encoding-safe (see _emit). An output-stream failure (e.g. a full or
    broken stdout) is an environment/tool failure that may surface as a nonzero tool exit; it is not a
    scanned-input verdict."""
    try:
        errors = []
        ...
        # Diagnostics: an unrecoverable output or scan failure fails closed to a cannot-evaluate exit 2. The
        # stderr note is itself best-effort and encoding-safe.
        try:
            _safe_write(sys.stderr, "cannot-evaluate: advisory run failed ({}: {}); fail-closed\n".format(
                type(exc).__name__, exc))
        except Exception:  # noqa: BLE001  stderr itself unwritable; still fail closed
            pass
        return 2
...
def self_test_main():
    ...
    try:
        ...
        (sel_root / "tools" / "real.py").write_text("import os\n", encoding="utf-8")
        ...
        with redirect_stdout(io.StringIO()):
            nul_errors, _, _ = scan([SI("tools/case_nul.py", b"import os\nx = 0\x00\n")])
        ...
        # (not 1); only an unrecoverable output failure routes to exit 2.
        ...
        # located capacity diagnostic), rather than be swallowed into a rc-0 advisory. Depth 400 parses but
