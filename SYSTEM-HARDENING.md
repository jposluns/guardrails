...
- **What the pack does:** the core hook returns a blocking `ask` for a `--mirror`, a command-local `-c remote.<name>.mirror=true` (or `--config-env`) mirror configuration in the same command, a forced `--all/--branches`, a `--prune` sweep (a wildcard or matching refspec, or `--all/--branches`), a forced or deleted wildcard destination, or the matching `:` refspec, and a `deny` for a literal force-update or deletion of `main` or `master`. A mirror mode or push refspec set in git config outside the command string (repository, global, or system config, or the `GIT_CONFIG_*` environment protocol) is not read offline and is a disclosed residual, as are a shell alias that obscures the push subcommand and a command-local `remote.<name>.push` refspec (which can force or delete through configuration with no judged flag). The offline check for a command-local mirror config has two paths: a parsed path that is complete for a parseable command (the tokenizer normalizes quotes), and, for a wrapped or unparseable command, a best-effort raw lexical fallback that asks on the common quoted-argument spellings it matches but does not catch arbitrary shell fragmentation of the option token, the key, or the value (an inherent lexical-scan boundary); network-side branch protection remains the backstop. In Claude Code that `ask` is a blocking human prompt; the pack ships no notice or advisory mode that downgrades it. These sweeps are destructive: `--mirror` and `--prune` delete remote refs absent locally, and a deleted remote branch is not automatically recoverable from the pushing side.
...
- **What the pack does:** ships the rule and the declaration convention: a verification over a committed state declares its revision and review set, the declared revision is derived from or validated against the authoritative task revision the review is for, and the dispatcher reconciles the declaration against the repository, over base derivation read from the commit's raw structure, before any verifier runs, withholding an irreconcilable or unauthoritative-revision dispatch as unverifiable. The pack cannot run that reconciliation itself; the verification dispatcher is adopter code the pack does not ship.
...
- **What you set up:** a review-declaration gate embedded in your verification dispatcher, running before any verifier is dispatched: it validates the declared revision against the authoritative task revision the review is for (the change-request head, the dispatched commit, or the task revision under gate), so a declarer-named revision reconciling to no authoritative task revision is withheld as unverifiable rather than reviewed; it resolves that revision to a commit and derives the base from the commit's raw structure, read so that no replacement ref, grafts entry, or commit-graph cache can rewrite it (the sole parent for an ordinary commit, the first parent for a merge, the empty tree for a root commit, and a cannot-evaluate rather than a root where a needed parent object is absent, as under a shallow or partial clone), with that same protection against substituted bytes extended to every git read the reconciliation and the verifier depend on, the changed-set diff against that derived base, the scoped working-tree cleanliness comparison, and the verifier's content read, each reading the exact stored object identity for the named revision with no substitution applied: replacement refs are disabled with GIT_NO_REPLACE_OBJECTS or an equivalent per-command --no-replace-objects, because a raw `cat-file` read still applies a replacement ref and rawness alone does not cover them (an exact-OID read via `cat-file --batch-all-objects` is replacement-insensitive); grafts are disabled by an empty graft file such as GIT_GRAFT_FILE set to an empty path, because --no-replace-objects does not disable a graft; and the commit-graph by a disabled commit-graph such as core.commitGraph=false; a raw object read ignores a graft and the commit-graph but not a replacement ref, so where substitution metadata could otherwise reach a dependent operation not so neutralized, or where the platform cannot guarantee a substitution-free read of the exact object identity, the dispatch is withheld as a cannot-evaluate, so no dependent read resolves to bytes other than the authoritative commit's and the dispatch is withheld unverifiable where that cannot be guaranteed; it reconciles the declared changed set against the git-derived diff of that revision against that derived base for equality (so a declared path absent from the commit, or a committed change absent from the declaration, is surfaced rather than silently absorbed, while a path the change needs that is omitted from both the commit and the declaration is the disclosed under-scoped-declaration residual it cannot reach); it checks scoped working-tree cleanliness over the review set covering the index, the tracked working tree, and untracked entries, with literal byte-safe path handling and a fail-closed cannot-evaluate on an unreadable or malformed probe; and it binds what the verifier reads to the declared revision, withholding an irreconcilable declaration as unverifiable.
...
- **What the pack does:** it ships the rule and a best-effort PreToolUse/Bash hook (`git_explicit_binding`) that returns a blocking `ask` when a directory shift (`cd`, `pushd`, or `popd`) precedes an untargeted git mutation or a whole-tree breadth operation, or when a whole-tree breadth operation precedes a publish (a push); an unparseable command falls back to a raw lexical scan of the common spellings. The hook detects a `cd`/`pushd`/`popd` directory shift only; it does NOT inspect an ambient `GIT_*` override (`GIT_DIR`/`GIT_WORK_TREE`) or a target-redirecting wrapper, alias, or script, so a mutation whose target those redirect is not caught by the hook. Its coverage is otherwise best-effort over an open shell grammar, a disclosed residual: arbitrary shell fragmentation, an unrecognized wrapper, or a construct the tokenizer cannot parse can carry a target shift past the matcher. The branch/tag mutation classifier is FAIL-SAFE: it treats a git branch/tag command as a mutation unless every option token positively resolves to a recognized read-neutral role with no create/rename/delete target, so an unrecognized or abbreviated form ASKS rather than being missed.
