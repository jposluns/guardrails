...
- **The gap:** all of that state, the sentinel and its anchor, the checkpoint and its init marker, the attestation register and its anchor, lives in a state directory the assistant itself can write. An actor running as that same account can delete the checkpoint and its marker together, or place a passing sentinel; and it can forge an attestation with filesystem write alone, either by appending a valid approved row to the register (uncommitted, so the git merge-base or anchor authority, which checks only the committed or anchored prefix, still passes) or by writing a snapshot bound to the real register tip. The yield reader re-anchors the snapshot and re-derives its refs from the register content, so a swapped register or a fabricated-refs snapshot no longer substantiates; an appended approved row still does. Code running as that actor cannot categorically prevent any of it; the in-code checks are best-effort bar-raisers.
